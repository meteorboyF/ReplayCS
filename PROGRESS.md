...
- Working branch: `fix/global-lesson-overlap`
- Working baseline: `bcba073` (`origin/main`, merge of PR #37)
- Teammate UI baseline: `d007dce` (PR #33, light-first theme and theme toggle), layered on the earlier `e75d3a9` redesign
- Production commit: `bcba073`
- Production: https://replaycs.vercel.app
- Deployment: `dpl_7mxFj7biQssToAQsLApKpjsi14XV` (`READY`)
- Stable deployment URL: https://replaycs-cvfg1xtby-meteorboy-f.vercel.app
- Recovery branch: `origin/feat/study-recap` at `2fb3489`
- Worktree: global overlap repair and regression tests in progress; local untracked `.claude/` remains excluded and must not be committed.
- Immediate next action: complete the full gate, merge and deploy the global overlap milestone, then build Heap/Priority Queue from the resulting `origin/main`.
...
- Responsive rules: global navigation becomes horizontally scrollable below 760px; LessonWorkspace collapses below 1000px and uses mobile tabs below 680px; playback remains in document flow so it cannot cover the active panel; page-specific grids must collapse before their fixed/min-content children overflow.
...
The current global overlap milestone adds a reusable bounding-box audit and stable lesson panel test IDs. Its 61/61 local browser checks pass across five viewports. It also fixes the mobile playback bar covering the visualizer and aligns the BST insert trace's initial semantic event with a real source line. See `docs/live-route-audit.md` for the implemented-route matrix. These changes are not yet merged or deployed.
...
- Current overlap milestone: 215/215 browser tests pass, including 61/61 dedicated layout checks across five viewports.
...
9. Global lesson overlap hardening: implementation and 61/61 focused browser audit complete on `fix/global-lesson-overlap`; full gate, PR, merge, and deployment pending.
