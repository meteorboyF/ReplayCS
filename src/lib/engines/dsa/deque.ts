...
export interface DequeMistakeMetadata {
  prompt: string;
...
function makePrediction(
  ...
  tag: string,
  wrongAnswer: TraceValue
): { prediction: PredictionChallenge; mistake: DequeMistakeMetadata } {
  return {
    prediction: {
      id,
      prompt,
...
function createTraceBuilder(
  ...
  const add: TraceBuilder['add'] = (
    ...
    if (state.readIndex !== null) visualFocus.push(`slot-${state.readIndex}`);
    if (state.writeIndex !== null) visualFocus.push(`slot-${state.writeIndex}`);
    for (const id of [state.head, state.tail, state.current, state.newNode])
...
// One checkpoint per deque operation, keyed by which end it touches.
function dequeCheckpoint(config: ResolvedConfig, builder: TraceBuilder) {
  const empty = builder.state.size === 0;
  const first = config.values[0];
  const last = config.values[config.values.length - 1];
  switch (config.operation) {
    case 'push-front':
      return makePrediction(
        'deque-lab:push-front:checkpoint',
        `push-front adds ${config.newValue}. Which end becomes the new front?`,
        'text',
        `${config.newValue} (front)`,
        `push-front inserts at the front, so ${config.newValue} becomes the new front while ${last ?? 'nothing'} stays at the back.`,
        'deque-end-confusion',
        `${config.newValue} (back)`
      );
    case 'push-back':
      return makePrediction(
        'deque-lab:push-back:checkpoint',
        `push-back adds ${config.newValue}. Which end becomes the new back?`,
        'text',
        `${config.newValue} (back)`,
        `push-back inserts at the back, so ${config.newValue} becomes the new back while ${first ?? 'nothing'} stays at the front.`,
        'deque-end-confusion',
        `${config.newValue} (front)`
      );
    case 'pop-front':
      return makePrediction(
        'deque-lab:pop-front:checkpoint',
        empty
          ? 'The deque is empty. What does pop-front return?'
          : 'Which value does pop-front remove and return?',
        empty ? 'text' : 'numeric',
        empty ? 'underflow' : (first as number),
        empty
          ? 'pop-front on an empty deque is underflow.'
          : `pop-front removes the front element ${first}, not the back.`,
        empty ? 'deque-overflow-underflow' : 'deque-end-confusion',
        empty ? 'null' : (last as number)
      );
    case 'pop-back':
      return makePrediction(
        'deque-lab:pop-back:checkpoint',
        empty
          ? 'The deque is empty. What does pop-back return?'
          : 'Which value does pop-back remove and return?',
        empty ? 'text' : 'numeric',
        empty ? 'underflow' : (last as number),
        empty
          ? 'pop-back on an empty deque is underflow.'
          : `pop-back removes the back element ${last}, not the front.`,
        empty ? 'deque-overflow-underflow' : 'deque-end-confusion',
        empty ? 'null' : (first as number)
      );
    case 'peek-front':
      return makePrediction(
        'deque-lab:peek-front:checkpoint',
        empty
          ? 'The deque is empty. What does peek-front return?'
          : 'What does peek-front return, and does size change?',
        empty ? 'text' : 'text',
        empty ? 'underflow' : `${first} (size unchanged)`,
        empty
          ? 'peek-front on an empty deque is underflow.'
          : `peek-front returns the front value ${first} and, unlike pop, leaves size unchanged.`,
        empty ? 'deque-overflow-underflow' : 'peek-vs-pop',
        empty ? 'null' : `${last} (size unchanged)`
      );
    case 'peek-back':
      return makePrediction(
        'deque-lab:peek-back:checkpoint',
        empty
          ? 'The deque is empty. What does peek-back return?'
          : 'What does peek-back return, and does size change?',
        empty ? 'text' : 'text',
        empty ? 'underflow' : `${last} (size unchanged)`,
        empty
          ? 'peek-back on an empty deque is underflow.'
          : `peek-back returns the back value ${last} and leaves size unchanged.`,
        empty ? 'deque-overflow-underflow' : 'peek-vs-pop',
        empty ? 'null' : `${first} (size unchanged)`
      );
  }
}
...
function runPushFront(builder: TraceBuilder, config: ResolvedConfig) {
  if (config.backing === 'circular-array') {
    ...
    if (builder.state.size === builder.state.capacity) {
      ...
      builder.add('push-resize', 'Resize array', 'Capacity exceeded', {
        allocation: 1,
        write: builder.state.size
      });
      return;
    }
    builder.add(
  ...
  if (config.backing === 'circular-array') {
    ...
    if (builder.state.tail) {
      builder.add(
        'update-tail-next',
        'Update tail next',
        'Point old tail next to new node',
        { write: 1 },
        (s) => {
...
function runPopFront(builder: TraceBuilder, config: ResolvedConfig) {
  if (config.backing === 'circular-array') {
    ...
    builder.add('check-new-head', 'Check new head', 'Check if deque is not empty after pop', {
      comparison: 1
    });
    if (builder.state.head) {
      builder.add(
...
function runPopBack(builder: TraceBuilder, config: ResolvedConfig) {
  if (config.backing === 'circular-array') {
    ...
    builder.add('check-new-tail', 'Check new tail', 'Check if deque is not empty after pop', {
      comparison: 1
    });
    if (builder.state.tail) {
      builder.add(
    ...
    switch (config.operation) {
      ...
      case 'peek-back':
        return [
          ...
          quad(
            'update-rear',
            '  int last = (q->rear - 1 + q->capacity) % q->capacity;',
            '  int last = (q.rear - 1 + q.capacity) % q.capacity;',
            '  int last = (q.rear - 1 + q.capacity) % q.capacity;',
            '    last = (q.rear - 1 + q.capacity) % q.capacity'
          ),
          quad(
            'return-val',
            '  return q->arr[last];',
      ...
      case 'pop-back':
        return [
          ...
          quad(
            ...
            '  if (q.tail == nullptr) return -1;',
            '  if (q.tail == null) return -1;',
            '    if q.tail is None: return -1'
          ),
          quad(
            'read-val',
...
function deriveDequeComplexity(caseId: string, backing: DequeBacking): string[] {
  if (caseId.endsWith('-resize')) {
    return [
      'The circular buffer is full, so a larger buffer is allocated and every element is copied.',
      'The copy loop runs once per element, so the work grows linearly with n.',
      'This single push is O(n) time and O(n) transient auxiliary space.'
    ];
  }
  return [
    backing === 'circular-array'
      ? 'A deque touches only the front or rear index and updates size — modular arithmetic, no shifting.'
      : 'A doubly linked deque relinks a fixed number of prev/next pointers at the chosen end.',
    'The number of operations does not depend on how many elements the deque holds.',
...
export function createDequeLesson(config: DequeConfig): TraceLesson {
  ...
  const complexityCase: SelectedCase = {
    ...baseCase,
    derivation: deriveDequeComplexity(baseCase.id, resolved.backing)
  };

  const state = initialRuntime(resolved);
  const builder = createTraceBuilder(resolved, complexityCase, state);
  // Compute the checkpoint while state is still initial (before the runner mutates size).
  const checkpoint = dequeCheckpoint(resolved, builder);
  ...
  // Attach the single prediction checkpoint + mistake metadata to the first step.
  if (checkpoint && builder.steps[0]) {
    builder.steps[0].prediction = checkpoint.prediction;
    builder.steps[0].metadata = {
      ...builder.steps[0].metadata,
      mistake: {
        prompt: checkpoint.mistake.prompt,
        wrongAnswer: checkpoint.mistake.wrongAnswer,
  ...
  const formatSource = (lang: SupportedLanguage): SourceLine[] => {
    ...
    let currentLine = 1;
    for (const sq of [...baseSource, ...opSource]) {
      lines.push({
        id: `${resolved.operation}-${resolved.backing}-${currentLine}`,
        number: currentLine,
        text: sq[lang],
        ...(sq.semantic ? { semanticOperationId: sq.semantic } : {})
      });
      currentLine++;
    }
    return lines;
  ...
  const backingLabel =
    resolved.backing === 'circular-array' ? 'circular array' : 'doubly linked list';

  return {
    id: 'deque-lab',
    subject: 'dsa-1',
    topic: 'Deque',
    title: `Deque Lab — ${metadata.label} (${backingLabel})`,
    description: metadata.description,
    difficulty: 'beginner',
    learningObjectives: [
      'Push and pop at both ends of a deque in O(1)',
      'Distinguish the front end from the back end during every operation',
      'Separate peek from pop, and overflow from underflow'
    ],
