<script lang="ts">
  ...
  const operationCompletionKeys: string[] = DEQUE_OPERATIONS.map((item) => String(item.id ?? ''));
  const operationCount = operationCompletionKeys.length;
  const languages: SupportedLanguage[] = ['c', 'cpp', 'java', 'python'];
  const complexityRows = [
    ['Insert Front', 'All cases', 'O(1)', 'O(1)'],
    ['Insert Rear', 'All cases', 'O(1)', 'O(1)'],
    ['Delete Front', 'All cases', 'O(1)', 'O(1)'],
    ['Delete Rear', 'All cases', 'O(1)', 'O(1)']
  ];
  const mistakeClinic = [
    ['deque-overflow', 'Insert to full deque', 'insert-front'],
    ['deque-underflow', 'Delete from empty deque', 'delete-front']
  ] as const;
  ...
  let newValue = $state(DEFAULT_DEQUE_CONFIG.newValue ?? 99);
  ...
  function metadataRecord(metadata: DequeOperationMetadata) {
    return metadata as unknown as Record<string, unknown>;
  }

  function operationMetadata(id: DequeOperation) {
    return (
      DEQUE_OPERATIONS.find((candidate) => metadataRecord(candidate).id === id) ??
      DEQUE_OPERATIONS[0]
    );
  }

  function metadataText(metadata: DequeOperationMetadata, ...keys: string[]) {
  ...
  function chooseOperation(next: DequeOperation) {
    operation = next;
    inputError = '';
    buildTrace();
  }
  ...
  function chooseClinicOperation(next: string) {
    const match = DEQUE_OPERATIONS.find(
      (candidate) => String(metadataRecord(candidate).id) === next
    );
    if (match) {
      chooseOperation(metadataRecord(match).id as DequeOperation);
      document.getElementById('deque-lab')?.scrollIntoView({ behavior: 'smooth' });
    }
  }
  ...
  function markOperationComplete() {
    ...
    if (!completedOperations.includes(id)) {
      ...
      localStorage.setItem('replaycs-deque-operations', JSON.stringify(completedOperations));
    }
    if (nextCompleted.length >= operationCount) {
      progress = completeLesson(progress, 'deque-lab', 35);
  ...
  function submitPrediction(correct: boolean, answer: string) {
    ...
    const evidenceId = `deque-lab:${operation}:${step.prediction.id}`;
    if (correct) {
  ...
  function recordMentorHint() {
    progress = recordHint(progress, 'deque-lab');
    saveProgress(progress);
  }
...
<svelte:head>
  ...
  <meta
    name="description"
    content="Execute double-ended queue operations one source line at a time."
  />
</svelte:head>
...
<header class="lesson-head">
  <div>
    ...
    <p class="eyebrow">Double-ended execution laboratory</p>
    <h1>Deque Lab</h1>
    <p>See every reference read and write before it changes memory.</p>
  </div>
  <div class="mastery" aria-label={`Deque operation mastery ${masteryPercent}%`}>
...
>
  <label class="operation-field"
    ...
    <select
      ...
      value={operation}
      onchange={(event) => chooseOperation(event.currentTarget.value as DequeOperation)}
    >
      {#each DEQUE_OPERATIONS as item}
...
<section class="language-notes lab-section" aria-labelledby="language-notes-title">
  ...
  <div>
    <article>
      <b>C</b>
      <p>Uses circular arrays or doubly linked lists.</p>
    </article>
    <article>
      <b>C++</b>
      <p><code>std::deque</code> implemented usually as array of arrays.</p>
    </article>
    <article>
      <b>Java</b>
      <p><code>Deque</code> interface with <code>ArrayDeque</code> or <code>LinkedList</code>.</p>
    </article>
    <article>
      <b>Python</b>
      <p><code>collections.deque</code> natively supports O(1) ops on both ends.</p>
    </article>
  </div>
</section>
...
<section class="matrix lab-section" aria-labelledby="matrix-title">
  <div class="section-heading">
    ...
    <h2 id="matrix-title">Deque complexity, with assumptions named</h2>
  </div>
