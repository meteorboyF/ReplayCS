<script lang="ts">
  ...
  import HeapVisualizer from '$lib/components/visualizers/HeapVisualizer.svelte';
  import {
    DEFAULT_HEAP_CONFIG,
    HEAP_OPERATIONS,
    createHeapLesson,
  ...
  // Operation × kind are separate mastery cases.
  const operationCompletionKeys: string[] = HEAP_OPERATIONS.flatMap((item) => [
    `${item.id}-max`,
    `${item.id}-min`
  ]);
  const operationCount = operationCompletionKeys.length;
  const complexityRows = [
    ['Peek root', 'Read index 0', 'O(1)', 'O(1)'],
    ['Insert', 'Sift up', 'O(log n)', 'O(1)'],
    ['Extract root', 'Sift down', 'O(log n)', 'O(1)'],
    ['Search (arbitrary)', 'Linear scan', 'O(n)', 'O(1)'],
    ['Build heap', 'Bottom-up heapify', 'O(n)', 'O(1)']
  ];
  const mistakeClinic = [
    ['heap-parent-child-index', 'Parent/child index math', 'insert'],
    ['heap-extract-replacement', 'What replaces the root', 'extract'],
    ['heap-search-order', 'Assuming a heap is searchable', 'search'],
    ['build-heap-start', 'Where build-heap starts', 'build']
  ] as const;

  let operation = $state<HeapOperation>(DEFAULT_HEAP_CONFIG.operation);
  let kind = $state<HeapKind>(DEFAULT_HEAP_CONFIG.kind);
  let valuesText = $state(DEFAULT_HEAP_CONFIG.values.join(', '));
  let value = $state(DEFAULT_HEAP_CONFIG.value ?? 60);
  let lesson = $state(createHeapLesson(config()));
  let index = $state(0);
  ...
  let operationMetadata = $derived(
    HEAP_OPERATIONS.find((candidate) => candidate.id === operation) ?? HEAP_OPERATIONS[0]
  );
  let needsValue = $derived(operation === 'insert' || operation === 'search');
  ...
  onMount(() => {
    ...
    try {
      const stored: unknown = JSON.parse(localStorage.getItem('replaycs-heap-operations') ?? '[]');
      completedOperations = [
        ...new Set(
          (Array.isArray(stored) ? stored : []).filter(
            (item: unknown): item is string =>
              typeof item === 'string' && operationCompletionKeys.includes(item)
          )
        )
      ];
    } catch {
  ...
  function config(): HeapConfig {
    return { operation, kind, values: parseValues(false), value };
  }

  function parseValues(validate = true) {
    const pieces = valuesText.split(',').map((item) => item.trim());
    if (pieces.some((item) => item === '' || !/^-?\d+$/.test(item)) || pieces.length > 10) {
      if (validate) inputError = 'Enter up to 10 comma-separated whole numbers.';
      return [...DEFAULT_HEAP_CONFIG.values];
    }
  ...
  function buildTrace() {
    ...
    try {
      lesson = createHeapLesson({ operation, kind, values, value });
      index = 0;
      submitted = [];
      mistake = null;
      playing = false;
  ...
  function chooseOperation(next: HeapOperation) {
    operation = next;
    inputError = '';
    buildTrace();
  }
  ...
  function chooseClinicOperation(next: string) {
    const match = HEAP_OPERATIONS.find((candidate) => candidate.id === next);
    if (match) {
      chooseOperation(match.id);
      document.getElementById('heap-lab')?.scrollIntoView({ behavior: 'smooth' });
    }
  }
  ...
  function markOperationComplete() {
    ...
    if (!completedOperations.includes(id)) {
      ...
      localStorage.setItem('replaycs-heap-operations', JSON.stringify(completedOperations));
    }
    if (nextCompleted.length >= operationCount) {
      progress = completeLesson(progress, 'heap-lab', 40);
  ...
  function submitPrediction(correct: boolean, answer: string) {
    ...
    const evidenceId = `heap-lab:${operation}:${step.prediction.id}`;
    if (correct) {
      ...
      const inferredStateKey = firstMutation?.entityId ?? 'cursor';
      ...
      const attempt: MistakeAttempt = {
        ...
        tag: authored?.tag ?? 'heap-parent-child-index',
  ...
  function recordMentorHint() {
    progress = recordHint(progress, 'heap-lab');
    saveProgress(progress);
  }
  ...
  function nodeCount(state: Record<string, TraceValue>) {
    return Array.isArray(state.heap) ? state.heap.length : 0;
  }
</script>
...
<svelte:head>
  <title>Heap Execution Lab · ReplayCS</title>
  <meta
    name="description"
    content="Execute heap peek, insert (sift-up), extract (sift-down), search, and O(n) build-heap one swap at a time, in both min and max heaps."
  />
</svelte:head>
...
<header class="lesson-head">
  <div>
    ...
    <p class="eyebrow">Binary heap laboratory</p>
    <h1>Heap Lab</h1>
    <p>Sift values up and down an array-backed tree, and see why building a heap is O(n).</p>
  </div>
  <div class="mastery" aria-label={`Heap operation mastery ${masteryPercent}%`}>
...
>
  <label class="operation-field"
    ...
    <select
      ...
      value={operation}
      onchange={(event) => chooseOperation(event.currentTarget.value as HeapOperation)}
    >
      {#each HEAP_OPERATIONS as item}
  ...
  <label
    >Heap kind
    <select
      aria-label="Heap kind"
      value={kind}
      onchange={(event) => {
        kind = event.currentTarget.value as HeapKind;
        buildTrace();
      }}
    >
  ...
  <label class="values-field"
    >Values
    <input bind:value={valuesText} aria-describedby="heap-help heap-error" />
  </label>
  {#if needsValue}
    <label>{operation === 'search' ? 'Target' : 'New value'}<input type="number" bind:value /></label>
  {/if}
  <button class="primary" type="submit">Build deterministic trace</button>
  <p id="heap-help">
    Node i has children 2i+1 and 2i+2. Build-heap treats your values as raw input; other operations
    start from a valid heap. Up to 10 integers.
  </p>
  {#if inputError}<p id="heap-error" class="error" role="alert">{inputError}</p>{/if}
</form>
...
<div id="heap-lab" class="lab">
  ...
  <main class="execution">
    <HeapVisualizer state={visibleState} />
    <TraceControls
      {index}
  ...
  <aside class="step-panel panel">
    ...
    {#if predictionResolved}
      ...
      {/key}
    {:else}
      <div class="mentor-lock">Resolve the deterministic checkpoint to unlock grounded AI help.</div>
    {/if}
  </aside>
</div>
...
<section class="matrix lab-section" aria-labelledby="matrix-title">
  <div class="section-heading">
    ...
    <h2 id="matrix-title">Heap complexity, with assumptions named</h2>
    <p>
